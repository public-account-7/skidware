--//mobile support modify by doitenroi.9941
-- made by portal
-- example at bottom
local Library = {};
do
...
function Library:MakeDraggable(Instance, Button, Cutoff)
	...
	UserInputService.InputChanged:Connect(function(input)
		...
		--
		function Library:GetAllConfigs()
    local configs = {}
    local folder = "config"
    ...
    if isfolder(folder) then
        ...
        for _, file in ipairs(files) do
            table.insert(configs, file:match("[^/\\]+$")) -- get only the filename
        end
    end
    ...
    local fileName = folder .. "/" .. name

    -- If user didn’t include extension, add .json
    ...
    if not isfile(fileName) then
        warn("Config not found:", fileName)
        return
    end

    local Config = readfile(fileName)
    ...
    for _, Value in pairs(Table) do
        ...
        if Table3[1] ~= "ConfigConfig_List" and #Table3 >= 2 then
            local Parsed = Table3[2]:sub(2, #Table3[2])
            if Parsed:sub(1, 3) == "rgb" then
                Parsed = string.split(Parsed:sub(5, #Parsed - 1), ",")
            elseif Parsed:sub(1, 3) == "key" then
                local Table4 = string.split(Parsed:sub(5, #Parsed - 1), ",")
                if Table4[1] == "nil" and Table4[2] == "nil" then
                    Table4[1], Table4[2] = nil, nil
                end
                Parsed = Table4
            elseif Parsed:sub(1, 4) == "bool" then
                Parsed = Parsed:sub(6, #Parsed - 1) == "true"
            elseif Parsed:sub(1, 5) == "table" then
                Parsed = string.split(Parsed:sub(7, #Parsed - 1), ",")
            elseif Parsed:sub(1, 6) == "string" then
                Parsed = Parsed:sub(8, #Parsed - 1)
            elseif Parsed:sub(1, 6) == "number" then
                Parsed = tonumber(Parsed:sub(8, #Parsed - 1))
            end
            Table2[Table3[1]] = Parsed
        end
    end

    for i, v in pairs(Table2) do
        if self.Flags[i] then
            if typeof(self.Flags[i]) == "table" and self.Flags[i].Set then
                self.Flags[i]:Set(v)
            elseif typeof(self.Flags[i]) == "function" then
                self.Flags[i](v)
            else
                self.Flags[i] = v
            end
...
Library:Connection(game:GetService("UserInputService").InputBegan, function(Input)
	if Window.Visible and (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch or Input.UserInputType == Enum.UserInputType.Gamepad1) then
